...
The response characateristics of the SPARS were examined at the level of each participant. Within each participant, we examined:
...
2. Whether response characteristics of the lower half (innoccuous sensation: -50 to 0)he upper half of the scale (noxious sensation: 0 to 50).
...
The random intercept only and random intercept and slope models were compared using the logliklihood test, and the better model taken foward. Diagnostics were only run on the final model, and examined level 1 residuals (conditional / fixed effects), and level 2 residuals (random effects) and outliers [^1].
...
The regression curve for the quadratic term shows some signs of deviating from a 0 slope, but otherwise the model specification (in terms of linearity) looks okay.
...
No serious obvious pattern to the scatter of residuals across any of the fixed effect terms. 
...
Looks like it meets the assumption that residuals are normally distributed.
...
Looks like level 2 residuals for the intercept and the slope meet the assumption that residuals are normally distributed.
...
Estimation of the variance component was undertaken by calculating relative variance change (RCV). RVC is close to zero when $unit_i$ does not have a large infuence on the variance component.
...
and explanatory variables using leverage.

```{r influence_leverage}
dotplot_diag(x = influence_leverage[, 1], 
             cutoff = "internal",
             name = "leverage") + 
    labs(title = 'Leverage: overall',
         subtitle = 'Cutoffs determined by measures of internal scaling',
         y = 'Leverage',
         x = 'Participant ID')
...
dotplot_diag(x = influence_leverage[, 3], 
             cutoff = "internal",
             name = "leverage") + 
    labs(title = 'Leverage: random effects',
         subtitle = 'Cutoffs determined by measures of internal scaling',
         y = 'Leverage',
         x = 'Participant ID')
...
Participants 5 and 12 were identified as having leverage. 
