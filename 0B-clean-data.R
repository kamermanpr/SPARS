...
SPAR_raw <- map_df(.x = files,
                   ~ as_data_frame(.x))

# Generate new columns
## Consolidate raw rating scale values into a single column
SPAR_raw %<>% mutate(rating = ifelse(!is.na(Rating_given_FEST),
...
## Create columns of FEST-equivalent values for CNRS_P and NRS_NP
SPAR_raw %<>% mutate(rating_eq = ifelse(scale == 'CNRS_P',
                                        yes = rating / 2,
...
SPAR_raw %<>% mutate(scale_combined = ifelse(scale == 'FEST',
                                             yes = scale,
                                             no = 'COMBINED'))

## Recalibrate NRS_NP to a -100 to 0 scale (-100 = no sensation, 0 = pain)
SPAR_raw %<>% mutate(rating_combined = ifelse(scale == 'NRS_NP',
                                              yes = rating - 100,
                                              no = rating))

## Center and scale rating data for FEST/COMBINED
SPAR_raw %<>% group_by(scale_combined) %>%
  mutate(rating_z_combined = scale(rating_combined))

## Center and scale rating data for FEST/CNRS_P/NRS_NP
SPAR_raw %<>% group_by(scale) %>%
  mutate(rating_z = scale(rating))

## Consolidate reaction time (RT) for each scale into a single column
SPAR_raw %<>% mutate(RT_msec = ifelse(!is.na(RT_FEST),
                                      yes = RT_FEST,
...
## Clean-up columns
SPAR_raw %<>% select(-starts_with("Rating_given_"),
                     -starts_with("eval_"),
                     -RT_FEST,
                     -RT_NP,
                     -RT_P)

# Rebase block_number to start at 1 and increase monotonically
SPAR_raw %<>% group_by(PID) %>%
  arrange(block_number, trial_number) %>%
  mutate(block_number = dense_rank(block_number))

# Rebase trial_number to start at 1 and increase monotonically
SPAR_raw %<>% group_by(PID) %>%
    arrange(block_number, trial_number) %>%
...
xtabs(~ PID + block_order, data = SPAR_raw)
...
SPAR_raw %<>% group_by(id) %>%
  mutate(intensity_rank = dense_rank(intensity))

# Order columns
SPAR_raw %<>% select(id, block_number, trial_number, intensity, intensity_rank,
...
# Save outputs
write_rds(x = SPAR_raw,
          path = 'data-cleaned/SPARS_B.rds')

write_csv(x = SPAR_raw,
