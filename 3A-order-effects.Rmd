---
title: "FEST 2015"
subtitle: "Effect of stimulus order and blocking on FEST rating"
...
knitr::opts_chunk$set(warning = FALSE,
                      ...
                      fig.width = 7,
                      fig.align = 'center',
                      fig.path = './figures/03-order-effects/')
```
...
data %<>%
  ...
  select(PID, block_number, trial_number, 
         intensity, rating, rating_positive) %>%
  # Format block and trial number type
  mutate(block_number = as.integer(block_number),
...
############################################################
data_lag <- data %>%
  # Drop unneeded columns (block_number and positive_intensity)
...
**Question:** Is the FEST rating for a given stimulus intensity dependent on the intensity of the preceding stimulus? 
...
Participant-level plots of FEST rating against the intensity of the preceding stimulus, across the range of stimuli used (1.00J to 4.00J). 
  ...
  mutate(plot = pmap(.l = list(data, trimean, unique(PID)),
                    ~ ggplot(data = ..1) +
                      ...
                      geom_hline(data = ..2, 
                                 ...
                                 size = 0.6,
                                 linetype = 2) +
                      labs(title = paste0(..3, ': FEST rating versus intensity of the previous stimulus, at different stimulus intensities'),
                           subtitle = 'Red line (dashed): Tukey trimean of FEST rating | Grey line (solid): Loess curve',
...
**Question:** Is the FEST rating for a given stimulus intensity associated with experimental block? 
  ...
  mutate(plot = pmap(.l = list(data, trimean, unique(PID)),
                    ~ ggplot(data = ..1) +
                      ...
                      geom_hline(data = ..2, 
                                 ...
                                 size = 0.6,
                                 linetype = 2) +
                      labs(title = paste0(..3, ': FEST rating versus experimental block number, at different stimulus intensities'),
                           subtitle = 'Red line (dashed): Tukey trimean of FEST rating | Grey line (solid): Loess curve',
                           y = 'FEST rating [-50 to 50]',
                           x = 'Block number') +
                      scale_y_continuous(limits = c(-50, 50)) +
